...
Usage:
    ...
    ./python.sh capture_hdr.py --usd /path/to/scene.usd \
        ...
        --res 4096 2048 --mode PathTracing
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="Capture equirectangular HDR panoramas from an Isaac Sim scene"
    )
    p.add_argument(
        "--usd", required=True,
    ...
    p.add_argument(
        "--out", default="./hdr_output",
        help="Output directory for .exr files (default: ./hdr_output)"
    )
    p.add_argument(
        "--pos", nargs=3, type=float, action="append",
        metavar=("X", "Y", "Z"),
        help=(
            "Capture position in world space. "
            "Repeat to capture from multiple positions. "
            "Default: 0 0 1.5"
        ),
    )
    p.add_argument(
        "--face-size", type=int, default=1024,
        help="Resolution of each cube face in pixels (default: 1024). "
             "Output EXR will be (4N x 2N)."
    )
    p.add_argument(
        "--mode", default="RaytracedLighting",
    ...
    p.add_argument(
        ...
        help="Run without GUI (default: show window)"
    )
    return p.parse_args()
...
from utils.hdr_capture_utils import capture_hdr_panorama
